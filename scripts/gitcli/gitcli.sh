...
add_in $temp1 "$(echo option_selected = $(choose_one "󰛐   show credentials" "   git push" "   git switch"))"
...
if [[ $(search_in $temp1 "option_selected" = 2) =~ "show credentials" ]]; then
  ...
  source "$SCRIPT_DIR/../scripts/gitcli/git_push.sh"
elif [[ $(search_in $temp1 "option_selected" = 2) =~ "git switch" ]]; then
  source "$SCRIPT_DIR/../scripts/gitcli/git_switch.sh"
fi
